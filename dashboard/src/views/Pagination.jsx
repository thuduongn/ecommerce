...
const Pagination = ({pageNumber,setPageNumber,totalItem,perPage,showItem}) => {
    ...
    const createBtn = () => {
        ...
        for (let i = startPage; i < endPage; i++) {
            btns.push(
                <li key= {i} onClick={()=>setPageNumber(i)} className={` ${pageNumber === i ? 'bg-[#4ea84d] shadow-lg shadow-indigo-300/50 text-white' : 'bg-white text-[#4D4D4D] hover:bg-[#4ea84d] shadow-lg hover:shadow-indigo-500/50 hover:text-white text-[#d0d2d6] border border-black-800'} w-[33px] h-[33px] rounded-full flex justify-center items-center cursor-pointer `}>
                    {i}                    
                </li>
            ) 
        }
